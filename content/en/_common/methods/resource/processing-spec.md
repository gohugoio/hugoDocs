...
compression
: {{< new-in 0.153.5 />}}
: The compression method to use when encoding the image. One of `lossless` or `lossy`. The `lossless` method is only applicable to WebP images. Default is `lossy`. See&nbsp;[details](/content-management/image-processing/#compression).
...
quality
: Applicable to JPEG and WebP images when using `lossy` [compression](#compression). Specify `qN` where `N` is a whole number between `1` and `100`, inclusive. Default is `75`. See&nbsp;[details](/content-management/image-processing/#quality).
