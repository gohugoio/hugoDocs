...
1. Set `publishDate` in the page [front matter](/content-management/front-matter/) to a datetime in the future. If you want the creation and publication datetime to be the same, it's also sufficient to only set `date` in the page front matter to a datetime in the future[^date-complementation].
...
[^date-complementation]: See [this comment](https://github.com/gohugoio/hugo/issues/3977#issuecomment-379154169) from hugo's creator _bep_ for an overview about in which order the different date variables are complemented by each other when not explicitly set.
