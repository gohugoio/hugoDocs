...
This is the default media type configuration in tabular form:
...
## Customize a media type

You can customize any of the default media types. For example, to switch the primary suffix for `text/html` from `html` to `htm`:
...
You can register new media types as needed. For example, to register a media type for an Atom feed:
...
Occasionally, you may need to define a media type without a suffix or delimiter. For example, [Netlify] recognizes configuration files named `_redirects` and `_headers`, which Hugo can generate using custom [output formats](g).
