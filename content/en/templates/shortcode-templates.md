...
Shortcodes can also be nested. In a nested shortcode, you can access the parent shortcode context with the [`.Parent` shortcode method][parent]. This can be very useful for inheritance of common shortcode parameters from the root.
...
Hugo's [`.Parent` shortcode method][parent] provides access to the parent shortcode context when the shortcode in question is called within the context of a *parent* shortcode. This provides an inheritance model for common shortcode parameters.
...
Use the [errorf](/functions/fmt/errorf) template func and [`.Position` shortcode method][position] to get useful error messages in shortcodes:
