...
and which may be retrieved by some other markdown hook, for example, the `render-heading` template.
Given this template file

{{< code file="layouts/_default/_markup/render-document.html" >}}
{{ if .Entering }} {{ .Page.Scratch.Set "todayMsg" "Choices have consequences" }}<div class="markup" title=" {{- .Page.Title -}}">{{ else }}</div>{{ end }}
{{< /code >}}
